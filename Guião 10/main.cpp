...
void computeNormal(int i, int j) {
    float* n = new float[3]; std::fill_n(n,3,0);
    float p1[3] = {(float)i,h(i,j-1),(float)j-1},
    p2[3]= {(float)i,h(i,j+1),(float)j+1},
    p3[3]= {(float)i-1,h(i-1,j),(float)j},
    p4[3]= {(float)i+1,h(i+1,j),(float)j},
    v1[3]={p2[0]-p1[0],p2[1]-p1[1],p2[2]-p1[2]},
    v2[3]={p4[0]-p3[0],p4[1]-p3[1],p4[2]-p3[2]};
    cross(v1,v2,n);
    normalize(n);
	...
	for (int i = 1; i < imageWidth - 2; i++) {
		for (int j = 1; j < imageWidth - 1; j++) {
			...
			if(tiled){
                ...
                texCoord.push_back(((float)i+1)/imageWidth);
                texCoord.push_back((float)j/imageWidth);
                texCoord.push_back((float)i/imageWidth);
                texCoord.push_back((float)j/imageWidth);
			}

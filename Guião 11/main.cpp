...
void prepareCylinder(float altura, float raio, int lados) {
    ...
    int vertex = 0;
    float texRad = 0.1875, centerTopX = 0.4375, centerY = texRad,centerBottomX = 0.8125,sideTop =1, sideBottom=0.375;
    float cylSeg = (float)1/(float)lados;
    float delta = 2.0f * _PI_ / lados;
    ...
    for (int i = 0; i < lados; ++i) {
        ...
        t[vertex*2 + 0] = centerTopX;
        t[vertex*2 + 1] = centerY;
        ...
        t[vertex*2 + 0] = centerTopX+(texRad *cos(  (_PI_/2)+(i * delta) ));
        t[vertex*2 + 1] = centerY+(texRad *sin( (_PI_/2)+(i * delta)));
        ...
        t[vertex*2 + 0] = centerTopX+(texRad *cos( (_PI_/2)+((i+1) * delta)));
        t[vertex*2 + 1] = centerY+(texRad *sin((_PI_/2)+ ((i+1) * delta)));
        ...
        t[vertex*2 + 0] = cylSeg * (i+1);
        t[vertex*2 + 1] = sideTop;
        ...
        t[vertex*2 + 0] = centerBottomX+(texRad *cos( (i+1) * delta));
        t[vertex*2 + 1] = centerY+(texRad *sin( (i+1) * delta));
        ...
        t[vertex*2 + 0] = centerBottomX+(texRad *cos( (i) * delta));
        t[vertex*2 + 1] = centerY+(texRad *sin( (i) * delta));
...
void drawFloor(float xmin, float xmax, float zmin, float zmax) {
    ...
    glBegin(GL_QUADS);
    glNormal3f(0, 1, 0);
    glVertex3f(xmax, 0, zmin);glTexCoord2f(1,1);
    glVertex3f(xmin, 0, zmin);glTexCoord2f(0,1);
    glVertex3f(xmin, 0, zmax);glTexCoord2f(0,0);
    glVertex3f(xmax, 0, zmax);glTexCoord2f(1,0);
    glEnd();
